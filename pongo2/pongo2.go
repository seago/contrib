...
func compile(opt Options) map[string]*pongo2.Template {
	...
	if opt.TemplateFileSystem == nil {
		opt.TemplateFileSystem = seago.NewTemplateFileSystem(seago.RenderOptions{
...
func renderHandler(opt Options, tplSets []string) seago.Handler {
	cs := seago.PrepareCharset(opt.Charset)
	ts := newTemplateSet()
	ts.Set(_DEFAULT_TPL_SET_NAME, &opt)

	var tmpOpt Options
	for _, tplSet := range tplSets {
		tplName, tplDir := seago.ParseTplSet(tplSet)
		tmpOpt = opt
		tmpOpt.Directory = tplDir
		ts.Set(tplName, &tmpOpt)
	}

	return func(ctx *seago.Context) {
		r := &render{
			TplRender: &seago.TplRender{
				ResponseWriter: ctx.Response,
				Opt: &seago.RenderOptions{
		...
		ctx.Render = r
		ctx.MapTo(r, (*seago.Render)(nil))
	}
}

// Pongoer is a Middleware that maps a seago.Render service into the Seago handler chain.
...
func (r *render) HTML(status int, name string, data interface{}, _ ...seago.HTMLOptions) {
	r.renderHTML(status, _DEFAULT_TPL_SET_NAME, name, data)
}

func (r *render) HTMLSet(status int, setName, tplName string, data interface{}, _ ...seago.HTMLOptions) {
	r.renderHTML(status, setName, tplName, data)
}

func (r *render) HTMLSetBytes(setName, tplName string, data interface{}, _ ...seago.HTMLOptions) ([]byte, error) {
	t, err := r.templateSet.Get(setName, tplName)
	if seago.Env == seago.DEV {
		opt := *r.opt
		opt.Directory = r.templateSet.GetDir(setName)
...
func (r *render) HTMLBytes(name string, data interface{}, _ ...seago.HTMLOptions) ([]byte, error) {
	return r.HTMLSetBytes(_DEFAULT_TPL_SET_NAME, name, data)
}

func (r *render) HTMLSetString(setName, tplName string, data interface{}, _ ...seago.HTMLOptions) (string, error) {
	p, err := r.HTMLSetBytes(setName, tplName, data)
	return string(p), err
}

func (r *render) HTMLString(name string, data interface{}, _ ...seago.HTMLOptions) (string, error) {
