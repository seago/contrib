...
func Test_PostgresProvider(t *testing.T) {
	Convey("Test postgres session provider", t, func() {
		opt := session.Options{
			Provider:       "postgres",
			ProviderConfig: "user=jiahuachen dbname=seago port=5432 sslmode=disable",
		}
		...
		Convey("Basic operation", func() {
			...
			m.Get("/", func(ctx *seago.Context, sess session.Store) {
				sess.Set("uname", "unknwon")
			})
			m.Get("/reg", func(ctx *seago.Context, sess session.Store) {
			...
			m.Get("/get", func(ctx *seago.Context, sess session.Store) {
				sid := sess.ID()
