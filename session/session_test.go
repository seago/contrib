...
func testProvider(opt Options) {
	Convey("Basic operation", func() {
		...
		m.Get("/", func(ctx *seago.Context, sess Store) {
			sess.Set("uname", "unknwon")
		})
		m.Get("/reg", func(ctx *seago.Context, sess Store) {
		...
		m.Get("/get", func(ctx *seago.Context, sess Store) {
			sid := sess.ID()
	...
	Convey("Regenrate empty session", func() {
		...
		req, err := http.NewRequest("GET", "/", nil)
		So(err, ShouldBeNil)
		req.Header.Set("Cookie", "seagoSession=ad2c7e3cbecfcf486; Path=/;")
		m.ServeHTTP(resp, req)
	})
...
func Test_Flash(t *testing.T) {
	Convey("Test flash", t, func() {
		...
		req, err = http.NewRequest("GET", "/get", nil)
		So(err, ShouldBeNil)
		req.Header.Set("Cookie", "seago_flash=error%3Derror%26info%3Dinfo%26success%3Dsuccess%26warning%3Dwarning; Path=/")
		m.ServeHTTP(resp, req)
	})
}
