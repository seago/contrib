...
type Store interface {
	...
	Destory(*seago.Context) error
	// RegenerateId regenerates a session store from old session ID to new one.
	RegenerateId(*seago.Context) (RawStore, error)
...
// Sessioner is a middleware that maps a session.SessionStore service into the Seago handler chain.
// An single variadic session.Options struct can be optionally provided to configure.
func Sessioner(opt Options) seago.Handler {
	...
	return func(ctx *seago.Context) {
		...
		// Get flash.
		vals, _ := url.ParseQuery(ctx.GetCookie("seago_flash"))
		if len(vals) > 0 {
			f := &Flash{Values: vals}
			...
			ctx.Data["Flash"] = f
			ctx.SetCookie("seago_flash", "", -1, opt.CookiePath)
		}

		f := &Flash{ctx, url.Values{}, "", "", "", ""}
		ctx.Response.Before(func(seago.ResponseWriter) {
			if flash := f.Encode(); len(flash) > 0 {
				ctx.SetCookie("seago_flash", flash, 0, opt.CookiePath)
			}
		})
...
func (m *Manager) Start(ctx *seago.Context) (RawStore, error) {
...
func (m *Manager) Destory(ctx *seago.Context) error {
	sid := ctx.GetCookie(m.opt.CookieName)
	if len(sid) == 0 {
		return nil
	}
...
// RegenerateId regenerates a session store from old session ID to new one.
func (m *Manager) RegenerateId(ctx *seago.Context) (sess RawStore, err error) {
	sid := m.sessionId()
...
func (f *Flash) set(name, msg string, current ...bool) {
	isShow := false
	if (len(current) == 0 && seago.FlashNow) ||
